...
robots.txt(검색엔진한테 "여긴 와도 돼, 여긴 안 돼"라고 알려주는 안내 표지판)는 사이트의 정문에 붙어 있는 출입 규칙표 같은 거야.
...
**핵심 규칙:**
- `User-agent: *`는 모든 크롤러에 적용되고, 특정 크롤러(`Googlebot`, `Bingbot`)별로 따로 규칙을 줄 수도 있어
- 여기서 중요한 건, `Disallow`는 "크롤링 차단"이지 "인덱싱 차단"이 아니라는 거야. 인덱싱까지 막으려면 `noindex` 메타 태그를 써야 해
- CSS/JavaScript 파일을 차단하면 Google이 페이지를 제대로 그려볼 수가 없어. `/wp-content/`나 `/assets/`를 통째로 차단하는 건 절대 하면 안 돼

**2026년 새로운 주의사항:**
- AI 크롤러(GPTBot, CCBot 등)가 등장하면서, 검색엔진 크롤러와 분리해서 관리해야 해
- `User-agent: GPTBot`과 `User-agent: Googlebot`을 별도로 설정하는 게 좋아
- 사이트맵은 여러 개 선언할 수 있어 (`Sitemap:` 지시문 복수 사용)
- 너무 광범위한 `User-agent: *` 규칙으로 과도하게 차단하지 않도록 주의해야 해

### 1.2 XML Sitemap 생성 및 제출

XML Sitemap(사이트의 중요 페이지 주소를 모아둔 지도)은 검색엔진한테 "우리 사이트에 이런 페이지들이 있어"라고 알려주는 목차 같은 거야.
...
**생성 방법:**
- CMS 플러그인: WordPress의 Yoast SEO, Rank Math 등이 자동으로 만들어줘
...
**최적화 규칙:**
- 인덱싱을 원하는 URL만 포함해야 해 (404, 301 리다이렉트, noindex 페이지는 빼야 돼)
- 50,000 URL 또는 50MB를 넘으면 사이트맵 인덱스 파일로 분할해야 해
- `<lastmod>`는 실제 콘텐츠가 바뀌었을 때만 업데이트해야 신뢰도가 유지돼

### 1.3 크롤 버짓(Crawl Budget) 관리

크롤 버짓은 쉽게 말해 검색엔진의 "체력"이야. Googlebot이 한 번 방문할 때 우리 사이트에서 읽어갈 수 있는 페이지 수에 한도가 있거든. 소규모 사이트(수백~수천 페이지)에서는 크게 신경 안 써도 되지만, 대규모 사이트(수만 페이지 이상)에서는 핵심 최적화 대상이야.

**크롤 버짓을 낭비시키는 주범들:**
- 무한 공간: 내부 검색 결과 페이지, 무한 필터 조합 같은 것들이 끝없이 URL을 만들어내
...
- 영구 삭제된 페이지는 404 또는 410 상태코드를 반환해야 해 (Google은 URL을 잊지 않지만, 404를 주면 재크롤링 빈도를 크게 낮춰)
- canonical 태그(같은 내용 중 "이게 원본이야"라고 알려주는 태그)로 중복 URL 정리
- 내부 링크 구조 개선으로 중요 페이지의 크롤링 깊이 축소

**주의:** robots.txt로 특정 페이지를 차단해도, 절약된 크롤 버짓이 다른 페이지로 자동으로 재배분되지는 않아. Google이 이미 서버 용량 한계에 도달한 경우에만 의미가 있는 거야.
...
- 소문자만 사용 (대소문자 혼용하면 중복 URL 발생할 수 있어)
...
- 모든 중요 페이지는 홈페이지에서 3클릭 이내에 도달할 수 있어야 해. 마트에서 원하는 물건이 3번 이내 코너를 돌면 찾을 수 있어야 하는 것처럼
- Hub-and-Spoke 구조: 상위 허브 페이지가 하위 상세 페이지로 연결되고, 상세 페이지는 관련 페이지로 횡적 연결하며 다시 상위로 연결하는 거야. 거미줄처럼 촘촘하게 엮인 구조지
- URL 깊이가 깊을수록 크롤링 우선순위가 낮아져

---

## Part 2: Core Web Vitals & 페이지 속도

Core Web Vitals(CWV)는 Google이 "이 사이트 사용자 경험 괜찮아?"를 판단하는 성적표 같은 거야. LCP, INP, CLS 이 세 가지 지표로 구성돼 있지.

### 2.1 LCP (Largest Contentful Paint)

LCP는 페이지에서 가장 큰 콘텐츠(메인 이미지, 제목 텍스트 등)가 화면에 뜨기까지 걸리는 시간이야. 쉽게 말해 "사용자가 '오, 뭔가 보인다!'고 느끼는 순간"까지의 시간이지.
...
**2025 Web Almanac 기준, 모바일 페이지의 62%만 Good LCP를 달성** — 가장 통과하기 어려운 CWV 지표야.

**개선 방법:**
- 히어로 이미지를 `<link rel="preload">`로 미리 불러오기. 식당에서 자주 나가는 메뉴 재료를 미리 손질해두는 것과 같은 원리야
...
- Next.js의 `next/image` 컴포넌트 활용 (자동 리사이징, 레이지 로딩, 모던 포맷 변환을 한 번에 해결해줘)

### 2.2 INP (Interaction to Next Paint)

INP는 2024년 3월에 FID(First Input Delay)를 완전히 대체한 지표야. 사용자가 버튼을 클릭하거나 뭔가 입력했을 때, 화면이 실제로 반응하기까지 걸리는 시간을 측정하는 거지. 버튼 눌렀는데 화면이 멍때리는 시간이 길면 점수가 나빠져.
...
**2026년 현재, 43%의 사이트가 200ms INP 기준을 통과하지 못함** — 가장 많은 사이트가 실패하는 CWV 지표야.

**개선 방법:**
- 긴 작업(Long Task)을 `requestIdleCallback()` 또는 `scheduler.yield()`로 잘게 쪼개기. 한 번에 큰 짐 하나를 옮기는 대신 작은 짐 여러 개로 나누는 거야
...
CLS는 페이지 로딩 중에 요소들이 갑자기 이리저리 밀리는 정도를 측정하는 거야. 글을 읽고 있는데 갑자기 광고가 끼어들어서 엉뚱한 버튼을 누르게 되는 그 짜증나는 현상 있잖아? 그걸 수치로 매기는 거지.
...
**개선 방법:**
- 이미지/비디오에 `width`와 `height` 속성 명시 (또는 CSS `aspect-ratio`). 자리를 미리 잡아두는 거야
...
Mobile-First Indexing(모바일 버전을 기준으로 사이트를 평가하겠다는 Google의 정책)은 2024년 7월 5일부로 100% 적용 완료됐어. 이제 데스크톱 전용 크롤링 예외는 아예 없고, Google은 오직 모바일 버전만 보고 인덱싱과 랭킹을 수행해.

쉽게 말해, 모바일에서 제대로 안 보이면 PC에서 아무리 잘 보여도 소용없다는 거야.

**핵심 요구사항:**
- 모바일과 데스크톱의 콘텐츠 동등성: 동일한 텍스트, 이미지, 메타 태그, 구조화된 데이터, 제목이 있어야 해
- 모바일에서 숨기기(`display: none`) 처리된 콘텐츠는 인덱싱 우선순위가 낮아질 수 있어
- Googlebot-Mobile이 접근할 수 없는 리소스가 없어야 해
...
**2025년 기준 글로벌 웹 트래픽의 60% 이상이 모바일 기기에서 발생**하니까, 반응형 디자인(화면 크기에 맞게 알아서 늘었다 줄었다 하는 디자인)이 사실상 표준이야. 한 벌의 옷이 사이즈에 맞게 조절되는 느낌이라고 보면 돼.
...
구조화된 데이터는 검색엔진과 AI한테 "이건 제품이고, 가격은 얼마고, 별점은 몇이야"처럼 라벨을 붙여주는 거야. 사람이 보면 알 수 있는 정보를 기계도 알 수 있게 이력서 형태로 정리해주는 거지.

Schema.org 어휘를 사용하고, Google은 **JSON-LD(구조화된 데이터를 JSON으로 써서 페이지에 붙여넣는 형식) 형식을 최우선으로 권장**해.
...
**JSON-LD의 장점:**
- HTML과 완전히 분리돼서 유지보수가 쉬워 (`<head>`의 `<script>` 태그에 위치)
- Microdata보다 구현/관리가 훨씬 간편해
- 2026년 기준 새로 구현한다면 JSON-LD가 유일한 선택지야

### 4.2 리치 스니펫 종류

리치 스니펫(검색 결과에서 별점, 가격, FAQ 같은 추가 정보가 예쁘게 표시되는 것)은 구조화된 데이터를 잘 넣어두면 검색 결과가 눈에 띄게 꾸며지는 효과야.
...
- **AI 검색 연동**: 2025년 3월에 Google, Microsoft 모두 생성형 AI 기능에 Schema 마크업을 사용한다고 공식 확인했어. ChatGPT도 구조화된 데이터를 사용해서 제품 표시를 결정하고 있지
- **CTR 영향**: Schema 마크업이 적용된 페이지는 리치 스니펫을 통해 CTR이 20~40% 향상돼
- **채택 현황**: 2024년 기준 4,500만 이상의 도메인이 4,500억 개 이상의 Schema.org 객체를 마크업하고 있어
- **Google의 2026년 1월 변경**: John Mueller가 2025년 11월에 일부 구조화된 데이터 유형의 지원 중단을 발표했어. 최신 문서 확인 필수야
...
HTTPS(브라우저와 서버 사이 통신을 암호화하는 보안 프로토콜)는 편지를 봉투에 넣고 잠금장치까지 건 거라고 보면 돼. Google이 공식 랭킹 시그널로 확인했고, 가벼운 신호이지만 동점 상황에서 결정적 차이를 만들어.

**2026년 핵심 변화:**
- Chrome 154 (2026년 10월): "항상 보안 연결 사용"이 기본값으로 전환돼. HTTP 사이트 방문 시 강력한 경고가 뜨게 돼
- 이제 모든 웹사이트는 HTTPS가 필수야. 이커머스나 금융 사이트만의 문제가 아니야

**체크리스트:**
- 전체 사이트 HTTPS 적용 (혼합 콘텐츠 mixed content 없어야 해)
...
Google이 접근성을 독립적인 랭킹 요소로 공식 확인하지는 않았지만, 접근성이 좋은 사이트는 SEO에서 일관되게 좋은 성과를 보여.

**접근성이 SEO에 미치는 영향:**
- 평균 세션 시간 22% 증가
- 이탈률 18% 감소
- 전환율 15% 향상
- Core Web Vitals와 직접 겹침: "Target Size" (버튼 크기) 기준은 모바일 우선 인덱싱, 페이지 경험 신호와 직결돼

결국 접근성이 좋다는 건 "누구나 편하게 쓸 수 있다"는 뜻이고, 그게 곧 좋은 사용자 경험이니까 SEO에도 당연히 좋은 거지.
...
SPA(Single Page Application, 페이지 전체를 새로 불러오지 않고 필요한 부분만 바꿔치기하는 웹앱)는 React, Vue, Angular 같은 프레임워크로 만들어. 빠르고 부드럽지만, 기본적으로 빈 HTML 껍데기를 보내고 브라우저에서 JavaScript로 내용을 채우는 방식이라 SEO에 문제가 생기는 거야.

왜 문제가 되냐면:

- **크롤링 지연**: Googlebot은 JavaScript를 실행할 수 있긴 한데, WRS(Google의 내장 브라우저 같은 렌더링 서비스)의 대기열에서 처리하느라 일반 HTML보다 인덱싱이 느려
- **2025년 12월 Google 렌더링 업데이트**: 비-200 HTTP 상태코드(404, 5xx)를 반환하는 페이지는 렌더링 파이프라인에서 아예 빠질 수 있게 됐어. JavaScript에 의존하는 404 페이지는 Googlebot이 내용 자체를 볼 수가 없는 거지
- **링크 크롤링 문제**: `<a href>` 없이 JavaScript 이벤트로만 네비게이션하면 크롤러가 링크를 발견하지 못해
- **메타 태그 동적 생성 실패**: CSR(브라우저에서 JavaScript로 화면을 그리는 방식) 시 `<title>`, `<meta description>` 등이 초기 HTML에 없을 수 있어

### 6.2 SSR vs SSG vs ISR

여기서 비유를 하나 들어보면 이해가 쉬워:
- **SSR = 주문 즉시 요리**: 손님이 올 때마다 그 자리에서 요리해서 내놓는 거야
- **SSG = 미리 만들어둔 도시락**: 빌드할 때 전부 만들어놓고, 손님이 오면 바로 꺼내주는 거지
- **ISR = 미리 만들어두되 주기적으로 리필하는 뷔페**: 기본은 만들어두고, 일정 시간마다 뒤에서 슬쩍 새 걸로 교체하는 방식이야
- **CSR = 재료만 보내고 손님이 직접 요리**: 서버는 빈 그릇만 주고, 브라우저가 알아서 조립하는 거야
...
- Next.js 16에서 라우팅/캐싱 개선으로 SSG + ISR + SSR 혼합 사용이 더 쉬워졌어
...
**Astro:**
- 기본적으로 Zero-JS 출력 (HTML 우선). JavaScript를 아예 안 보내는 게 기본값이라 SEO에 최강이야
...
hreflang은 검색엔진한테 "이 페이지의 한국어 버전은 여기, 영어 버전은 여기야"라고 알려주는 태그야. 다국어 사이트에서 각 나라 사용자에게 맞는 버전이 검색 결과에 노출되도록 해주는 거지.
...
1. **양방향 참조**: 한국어 페이지가 영어 페이지를 참조하면, 영어 페이지도 반드시 한국어 페이지를 참조해야 해. 악수는 양손으로 하는 거야
2. **자기 참조 태그**: 각 페이지는 자기 자신도 hreflang에 포함해야 해
3. **x-default 설정**: 어떤 언어에도 매칭되지 않는 사용자를 위한 기본 페이지 지정
4. **유효한 ISO 코드**: 언어는 ISO 639-1 (ko, en, ja), 지역은 ISO 3166-1 Alpha 2 (KR, US, JP)
5. **URL 정규화**: hreflang에 사용하는 URL은 canonical URL과 일치해야 해

### 7.4 흔한 실수

- **75%의 국제 사이트에 hreflang 오류가 있어** (양방향 참조 누락이 가장 흔함)
- 단방향 참조: A→B는 있는데 B→A가 없는 경우
- 자기 참조 누락
- 잘못된 언어/지역 코드 (`kr`이 아니라 `ko`, `jp`가 아니라 `ja`야)
- canonical 태그와 hreflang URL 불일치
- **hreflang은 힌트일 뿐이야**: Google은 hreflang을 절대적 지시가 아닌 참고 신호로 처리한다고 명시했어
...
ccTLD(`.kr`, `.jp`처럼 국가 코드가 붙은 도메인)는 강력하지만 비싸고, 대부분의 경우 **서브디렉토리 방식이 가장 실용적**이야.

---

## 핵심 인사이트

1. **2026년 Technical SEO는 "검색엔진"만의 문제가 아니야.** 구조화된 데이터와 렌더링 전략이 Google AI Overviews, ChatGPT, Perplexity 등 AI 검색의 콘텐츠 인용에 직접 영향을 미쳐. Schema.org 마크업은 이제 리치 스니펫을 넘어 "AI한테 발견되기 위한 필수 인프라"인 거지.

2. **INP가 Core Web Vitals의 새로운 병목이야.** LCP와 CLS는 개선 패턴이 잘 정립됐지만, INP는 43%의 사이트가 실패하고 있고 깊은 기술적 변경(메인 스레드 최적화, Long Task 분할, 서드파티 스크립트 관리)이 필요해.

3. **JavaScript SEO에서 "빈 HTML 껍데기" 시대는 끝났어.** Google의 2025년 12월 렌더링 업데이트로 비-200 상태코드 페이지의 렌더링이 제외될 수 있게 되면서, SSR/SSG/ISR 전략의 중요성이 더 커졌어. Next.js, Nuxt.js, Astro 같은 프레임워크가 제공하는 하이브리드 렌더링을 적극 활용해야 해.
...
| 용어 | 쉬운 설명 |
|------|----------|
| robots.txt | 검색엔진 크롤러한테 "여긴 들어와도 돼, 여긴 안 돼"를 알려주는 안내 표지판 같은 텍스트 파일이라고 보면 돼 |
...
| Core Web Vitals | Google이 "이 사이트 사용자 경험 괜찮은가?"를 판단하는 핵심 성능 지표 3개 세트 (LCP, INP, CLS). 성적표 같은 거라고 보면 돼 |
...
| CLS (Cumulative Layout Shift) | 페이지 로딩 중에 요소들이 갑자기 이리저리 밀리는 정도. 글 읽고 있는데 갑자기 광고가 끼어들어서 다른 버튼을 누르게 되는 그 짜증나는 현상의 수치라고 보면 돼 |
...
| JSON-LD | 구조화된 데이터를 HTML에 넣을 때 쓰는 형식 중 하나. 쉽게 말해 검색엔진이 읽을 수 있는 이력서를 JSON으로 써서 페이지에 붙여넣는 거라고 보면 돼 |
...
| WRS (Web Rendering Service) | Google이 JavaScript로 만든 페이지를 실제로 브라우저처럼 실행해서 내용을 확인하는 서비스. 쉽게 말해 Google의 내장 브라우저라고 보면 돼 |
| 레이지 로딩 (Lazy Loading) | 화면에 보이는 부분만 먼저 로드하고, 스크롤해서 내려가면 그때 나머지를 불러오는 방식. 뷔페에서 한꺼번에 다 가져오지 않고 먹을 만큼만 가져오는 느낌 |
| hydrate | SSR로 서버에서 받은 정적 HTML에 JavaScript 이벤트와 상태를 연결해서 인터랙티브하게 만드는 과정. 마네킹(정적 HTML)에 생명을 불어넣는 거라고 보면 돼 |
