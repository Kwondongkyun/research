...
| 용어 | 쉬운 설명 |
|------|----------|
| **Short-tail 키워드** | "SEO", "마케팅"처럼 1~2단어짜리 짧은 검색어. 검색하는 사람은 많지만 경쟁이 치열해서, 대형 쇼핑몰 앞 노점상 자리 잡기 같다고 보면 돼 |
| **Long-tail 키워드** | "블로그 SEO 최적화 방법 2026"처럼 3단어 이상의 구체적인 검색어. 검색량은 적지만 딱 그걸 찾는 사람이 오니까 전환율이 높아. 골목 맛집 같은 느낌 |
| **검색 의도 (Search Intent)** | 사람이 검색할 때 머릿속에 있는 진짜 목적. Google은 이걸 4가지로 나눠 |
| **ㄴ Informational** | "~란 뭐야?", "~하는 방법" 같은 정보 탐색형. 쉽게 말해 궁금해서 검색하는 거 |
| **ㄴ Navigational** | "네이버 로그인", "유튜브"처럼 특정 사이트에 가려고 검색하는 거. 주소창 대신 검색창 쓰는 거라고 보면 돼 |
...
| **DA (Domain Authority)** | 사이트 전체의 "신뢰도 점수" 같은 거. 0~100 사이 숫자인데, 높을수록 Google이 이 사이트를 믿고 상위에 올려줄 가능성이 커져 |
| **SV (Search Volume)** | 한 달에 그 키워드를 몇 명이나 검색하는지 나타내는 숫자. 쉽게 말해 "이 키워드 인기가 얼마나 되는지" 보여주는 지표 |
| **LSI 키워드** | 원래는 1980년대 기술 용어인데, SEO에서는 "의미적으로 관련된 단어들"이라는 뜻으로 통용돼. "커피"를 쓰면 "원두", "카페인", "드립" 같은 연관어를 함께 넣으라는 거 |
| **Title Tag** | 브라우저 탭이랑 검색 결과에 뜨는 페이지 제목. 가게 간판이라고 보면 돼 — 첫인상을 결정하는 핵심 요소 |
...
| **Alt Text** | 이미지가 안 보일 때 대신 표시되는 설명 텍스트. 시각장애인용 스크린리더도 이걸 읽어주고, Google 이미지 검색에도 영향을 줘 |
| **Internal Linking (내부 링크)** | 내 사이트 안에서 페이지끼리 서로 연결하는 링크. 백화점 안내 표지판처럼 방문자와 Google이 사이트 구조를 파악하게 도와줘 |
| **앵커 텍스트** | 링크가 걸린 클릭 가능한 텍스트 부분. "여기를 클릭"보다 "On-page SEO 가이드"처럼 내용을 설명하는 텍스트가 훨씬 좋아 |
| **토픽 클러스터 (Topic Cluster)** | 하나의 큰 주제를 중심으로 관련 세부 글들을 거미줄처럼 엮는 콘텐츠 구조. Google한테 "이 사이트가 이 주제 전문가야"라고 어필하는 전략 |
| **필러 페이지 (Pillar Page)** | 하나의 큰 주제를 총정리하는 "메인 가이드" 페이지. 여기서 세부 글들로 링크가 뻗어나가는 허브 역할을 해 |
...
| **AI Overview / SGE** | Google 검색 결과 맨 위에 AI가 자동으로 요약해주는 답변 박스. 이게 뜨면 아래 사이트들의 클릭이 확 줄어들어서 SEO 판도가 바뀌고 있어 |
| **Featured Snippet** | 검색 결과 맨 위에 박스로 하이라이트되는 답변 영역. "0번 검색 결과"라고도 불려. 여기 뜨면 클릭률이 확 올라가 |
...
| **GEO (Generative Engine Optimization)** | ChatGPT, Gemini 같은 AI 검색 엔진에서 우리 콘텐츠가 인용되도록 최적화하는 것. SEO의 AI 시대 버전이라고 보면 돼 |
| **에버그린 콘텐츠** | 시간이 지나도 가치가 떨어지지 않는 상시 유효한 콘텐츠. "Python 입문 가이드" 같은 거. 반대로 "2026년 트렌드"는 금방 낡아 |
| **Scaled Content Abuse** | AI로 저품질 콘텐츠를 대량으로 찍어내는 행위. Google이 명시적으로 페널티를 때리는 대상이야. 양으로 승부하려다 사이트 자체가 매장될 수 있어 |
| **WebP / AVIF** | 기존 JPEG·PNG보다 용량이 25~50% 작으면서 화질은 비슷한 차세대 이미지 포맷. 페이지 로딩 속도를 빠르게 해줘 |
