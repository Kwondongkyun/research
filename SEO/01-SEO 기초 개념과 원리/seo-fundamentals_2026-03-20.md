...
| 용어 | 쉬운 설명 |
|------|----------|
| **SEO (Search Engine Optimization)** | 검색엔진 최적화. 구글 같은 검색엔진에서 내 사이트가 위에 뜨게 만드는 작업이라고 보면 돼 |
| **SERP (Search Engine Results Page)** | 구글에서 뭔가 검색했을 때 나오는 그 결과 페이지 자체를 말하는 거야 |
| **유기적 트래픽 (Organic Traffic)** | 광고비 안 쓰고 순수하게 검색해서 들어오는 방문자. 쉽게 말해 '공짜 손님'이야 |
| **CTR (Click-Through Rate)** | 클릭률. 검색 결과에 내 사이트가 100번 보였는데 10번 클릭됐으면 CTR 10%라고 보면 돼 |
| **CAGR** | 연평균 성장률. "매년 평균 몇 %씩 커졌어?"를 한 숫자로 표현한 거야 |
| **크롤링 (Crawling)** | 검색엔진 봇이 웹사이트를 돌아다니면서 페이지를 수집하는 과정. 도서관 사서가 새 책을 찾으러 서점을 돌아다니는 것과 비슷해 |
| **인덱싱 (Indexing)** | 크롤링으로 찾은 페이지를 검색엔진 데이터베이스에 등록하는 과정. 사서가 찾아온 책을 분류해서 서가에 꽂아두는 거라고 보면 돼 |
| **랭킹 (Ranking)** | 검색했을 때 어떤 페이지를 몇 번째로 보여줄지 순서를 매기는 것. 시험 성적표에서 등수 매기는 거랑 같아 |
| **Googlebot** | 구글이 운영하는 자동 크롤링 로봇. 24시간 쉬지 않고 전 세계 웹사이트를 돌아다니면서 새로운 콘텐츠를 찾아다니는 녀석이야 |
| **robots.txt** | 웹사이트 입구에 붙여놓는 '출입 규칙표' 같은 파일. "여기는 들어와도 돼, 저기는 오지 마" 하고 크롤러한테 알려주는 거야 |
| **사이트맵 (sitemap.xml)** | 내 사이트의 '전체 지도' 파일. 크롤러한테 "우리 사이트에 이런 페이지들이 있어"라고 목록을 쫙 알려주는 거야 |
| **nofollow** | 링크에 붙이는 태그인데, 크롤러한테 "이 링크는 따라가지 마"라고 말하는 거야. 쉽게 말해 '추천은 안 함' 스티커 같은 거 |
| **noindex** | 페이지에 붙이는 태그로 "이 페이지는 검색 결과에 보여주지 마"라는 뜻이야. 비공개 문서에 '열람 금지' 딱지 붙이는 것과 같아 |
| **canonical (정규화)** | 같은 내용의 페이지가 여러 개 있을 때 "이게 원본이야!"라고 검색엔진한테 알려주는 거야. 쌍둥이 중에 대표 한 명을 지정하는 느낌 |
| **thin content** | 내용이 너무 빈약하거나 가치 없는 페이지. 쉽게 말해 '속 빈 강정' 같은 콘텐츠야 |
| **Mobile-First Indexing** | 구글이 PC 버전 대신 모바일 버전을 기준으로 사이트를 평가하는 방식. 요즘은 폰으로 더 많이 검색하니까 모바일이 '본캐'가 된 거야 |
| **키워드 스터핑 (keyword stuffing)** | 검색 순위 올리겠다고 키워드를 억지로 도배하는 블랙햇 수법. "맛집 맛집 맛집 맛집..." 이런 식으로 쓰는 거라 보면 돼 |
| **링크 스킴/링크 팜** | 순위 조작 목적으로 가짜 사이트들끼리 서로 링크를 주고받는 네트워크. 쉽게 말해 자기들끼리 '가짜 추천서'를 돌려쓰는 거야 |
| **시맨틱 검색 (semantic search)** | 키워드를 글자 그대로 매칭하는 게 아니라 의미와 맥락을 이해해서 검색하는 방식. "배 아파"를 검색하면 과일 배가 아니라 복통을 이해하는 거야 |
| **백링크 (Backlink)** | 다른 사이트가 내 사이트 링크를 걸어주는 것. 쉽게 말해 '추천서' 같은 거야. 좋은 데서 받을수록 효과가 커 |
| **Core Web Vitals** | 구글이 정한 웹사이트 성능 지표 3가지(로딩 속도, 반응성, 시각 안정성). 쉽게 말해 "이 사이트 빠르고 쾌적해?" 성적표라고 보면 돼 |
| **Schema Markup** | 검색엔진이 콘텐츠를 더 잘 이해하도록 붙이는 구조화된 태그. 음식에 영양성분표 붙이는 것처럼, 콘텐츠에 '이건 레시피야', '이건 리뷰야' 라벨을 달아주는 거야 |
| **링크 주스 (link juice)** | 한 페이지가 다른 페이지에 링크를 걸면 전달되는 'SEO 파워' 같은 거. 쉽게 말해 인기 있는 친구가 나를 소개해주면 나도 인기가 오르는 원리야 |
| **E-E-A-T** | Experience(경험), Expertise(전문성), Authoritativeness(권위성), Trustworthiness(신뢰성)의 약자. 구글이 "이 콘텐츠 믿을 만해?"를 판단하는 4가지 기준이야 |
| **YMYL** | Your Money Your Life의 약자. 건강, 금융, 법률처럼 잘못된 정보가 사람의 돈이나 생명에 직접 영향 줄 수 있는 주제를 말해. 이런 분야는 E-E-A-T 기준이 특히 빡빡해 |
| **PBN (Private Blog Network)** | 백링크를 인위적으로 만들기 위해 운영하는 가짜 블로그 네트워크. 쉽게 말해 자작극으로 추천서를 양산하는 블랙햇 수법이야 |
| **콘텐츠 팜 (content farm)** | 검색 트래픽만 노리고 저품질 글을 대량 생산하는 사이트. 질보다 양으로 승부하는 '기사 공장'이라고 보면 돼 |
| **클로킹 (Cloaking)** | 검색엔진한테는 A라는 콘텐츠를 보여주고, 실제 사용자한테는 B를 보여주는 속임수. 쉽게 말해 '이중 메뉴판' 사기야 |
| **PageRank** | 구글 창립 때 만든 원조 알고리즘. "다른 사이트들이 얼마나 많이 링크를 걸어주느냐"로 페이지 중요도를 매기는 방식. 논문 인용 수로 학자 영향력 판단하는 거랑 같은 원리야 |
