...
GEO 최적화의 핵심은 "AI가 인용하기 좋은 콘텐츠"를 만드는 거임. JSON-LD(검색엔진이 읽을 수 있는 구조화 태그)가 있는 페이지는 AI 인용 확률이 2.8배 높고, 통계/출처를 포함하면 가시성이 30~40% 올라감. llms.txt(AI한테 내 사이트를 소개하는 안내서 파일)라는 새로운 표준이 등장했고, 플랫폼별로 인용 도메인 중복률이 11%밖에 안 돼서 맞춤 전략이 필수임. geo-seo-claude 같은 오픈소스 도구로 무료 GEO 감사가 가능하고, GEO 적용 사례에서 AI 트래픽이 YoY 527% 증가한 결과가 나오고 있음.
...
JSON-LD(검색엔진이 읽을 수 있는 구조화 태그)는 Google이 공식 권장하는 Schema 마크업 구현 형식임. 쉽게 말하면, 웹페이지에 "이 페이지의 제목은 이거고, 저자는 누구고, 별점은 몇 점이야"라고 기계가 읽을 수 있는 라벨을 붙여주는 거임. AI 시스템과의 관계가 아주 직접적임:
...
**가장 효과적인 Schema 타입:**
- **FAQPage**: AI 인용에 가장 효과적임. AI가 본질적으로 Q&A 시스템이니까, FAQ 형식이랑 딱 맞음
...
여기가 GEO의 핵심 차별점임. 패시지 단위 최적화(페이지가 아니라 문단 하나하나를 AI가 인용하기 좋게 만드는 것)가 핵심임. 비유하자면, **책 전체가 아니라 밑줄 친 한 문단이 인용되게 만드는 거임.** AI는 페이지 전체를 가져가지 않고, 특정 문단이나 주장만 쏙 뽑아감.

**실전 가이드라인:**
- **BLUF(결론부터 먼저 쓰는 방식)**: 각 섹션의 첫 40-60단어에 핵심 답변을 배치함. "서론-본론-결론" 말고 "결론-근거-부연" 순서로 쓰는 거임
...
"출처 달고 숫자 넣으면 AI가 좋아한다"는 걸 연구로 증명한 결과임:
...
- 모든 주장에 수치를 부여하고, 출처와 발행 연도를 명시해야 함
- AI는 최신 정보를 선호하니까 발행/업데이트 날짜가 중요함

### 기술적 최적화: robots.txt AI 크롤러 관리

2026년 권장 전략은 **"훈련은 차단, 검색은 허용"**임. AI 검색에서 인용되려면 검색용 크롤러는 열어줘야 하지만, 내 콘텐츠로 AI 모델을 훈련시키는 건 막겠다는 뜻임.
...
토픽 권위는 "이 사이트는 이 주제에 대해 정말 잘 아는 곳이구나"라고 AI가 인식하게 만드는 거임. 한두 편 글 쓴다고 되는 게 아니라, 하나의 주제를 중심으로 꾸준히 쌓아야 함.
...
- 중요 콘텐츠는 최소 **3개월마다 업데이트** (AI는 최신 콘텐츠를 편애함)
...
llms.txt(AI한테 내 사이트를 소개하는 안내서 파일)는 Jeremy Howard가 제안한 표준임. 웹사이트 루트(`/llms.txt`)에 마크다운 파일을 놓아서, AI가 내 사이트를 한눈에 파악할 수 있게 도와주는 거임.

robots.txt가 "크롤러한테 출입 규칙을 알려주는 파일"이라면, llms.txt는 **"AI한테 보내는 사이트 소개서"**임. "우리 사이트는 이런 곳이고, 중요한 페이지는 여기 있어"라고 AI한테 직접 알려주는 거임.
...
비유하자면, llms.txt는 **도서관 카탈로그**(어떤 책이 있는지 목록)이고, llms-full.txt는 **도서관의 모든 책을 통째로 넘기는 것**임.
...
여기서 중요한 건, 각 플랫폼이 완전히 다른 소스를 인용한다는 거임. 마치 같은 질문을 세 명의 전문가한테 했는데, 각각 다른 책을 참고서로 꺼내드는 것과 같음.
...
- **Bing 순위가 핵심임** (Google 순위와는 상관없음 — ChatGPT 인용의 90%가 Google 21위 이하에서 나옴)
- Wikipedia 스타일의 백과사전적, 정의형 콘텐츠를 좋아함
- 구조화된 정보, 명확한 정의, 팩트 중심 서술이 유리함

### Google AI Overviews 최적화

- 기존 Google SEO를 잘해왔다면 유리함. 상위 랭킹이 AI Overviews 인용에도 영향을 줌
- 멀티모달 콘텐츠(영상, 이미지) 활용이 유리함
- E-E-A-T 시그널을 특히 중시함
- 쿼리당 인용 소스가 3개뿐이라 경쟁이 치열함

### Perplexity 최적화

- **최신성이 가장 중요함** — 인용의 76.4%가 30일 내 업데이트된 콘텐츠임
- Reddit, 커뮤니티 포럼 등 실사용자 콘텐츠를 중시함
- 전환율 14.2%로 Google(2.8%) 대비 5배지만, 절대 트래픽은 Google이 345배

### Claude 최적화

- 고품질, 깊이 있는 분석적 콘텐츠를 선호함
- 학술적 출처, 공식 문서, 전문가 의견에 높은 가중치를 줌
- 구조화된 데이터와 명확한 사실 기반 서술이 유리함

---

## Part 4: E-E-A-T와 GEO의 관계

2026년에 E-E-A-T는 단순 가이드라인에서 **AI 검색의 최우선 랭킹 팩터**로 격상됐음. 쉽게 말하면, "이 글 쓴 사람이 진짜 전문가 맞아?"를 AI가 엄격하게 따지기 시작한 거임.

- **E-E-A-T는 자격 요건(eligibility)임.** 이게 없으면 아예 후보에 못 들어감. SEO/GEO는 자격 있는 콘텐츠 중에서 **누구를 선택할지(selection)** 결정하는 기준임
- AI 에이전트는 Entity-Identity Protocol(AI가 저자의 자격을 교차 검증하는 시스템)을 통해 저자 자격을 ORCID, LinkedIn, 정부 레지스트리 등에서 교차 검증함. 마치 면접관이 이력서를 레퍼런스 체크하는 것처럼
- **AI 인용의 48%가 Reddit, YouTube, 전문 포럼 등 커뮤니티 플랫폼에서 발생함** — 자사 웹사이트만으로는 부족하다는 뜻임
- Wikipedia, 업계 백서, 학술 논문 등 중립적 제3자가 일관되게 인용하면 브랜드 권위가 AI의 핵심 지식에 새겨짐
...
기존 SEO가 "Google 몇 위냐"를 봤다면, GEO는 "AI가 우리를 얼마나 언급하냐"를 봐야 함:
...
Claude Code 스킬로 설치되는 무료 GEO 분석 도구임. 에이전시가 월 200~1200만원 받고 하는 감사를 무료로 돌릴 수 있음.
...
**감사 프로세스:** 5개 서브에이전트가 병렬로 돌아가면서 (AI 가시성, 플랫폼 분석, 기술 SEO, 콘텐츠 품질, 스키마 마크업) 분석하고 → GEO 복합점수(0-100)를 산출함
...
숫자가 말해주는 거임. AI 검색 트래픽이 폭발적으로 성장하고 있음:
...
**Xponent21:**
- 유기 트래픽 **YoY 4,162% 성장** (이건 정말 미친 수치임)
...
- 해당 트래픽의 **27%가 Sales-Qualified Lead로 전환** — 일반 트래픽보다 훨씬 질이 좋다는 뜻임
...
- 기업 사용자는 LLM에 더 길고 맥락적이며 리스크를 고려한 프롬프트를 사용함

**이커머스:**
- "마라톤에 가장 좋은 러닝화?" 같은 대화형 질문에 직접 답변하는 콘텐츠가 유리함
- Product, Review Schema 활용, 사용자 리뷰와 커뮤니티 멘션이 중요함

**헬스케어/규제 산업:**
- AI 필터가 특히 엄격해서 — 정확하고 잘 구조화된 문서만 선호함
- 학술 인용, 공식 가이드라인 참조 필수
- Author Schema에 의료 자격 증명 포함해야 함
...
시장 규모를 보면 GEO가 얼마나 급격히 커지고 있는지 체감이 됨:
...
1. **GEO는 SEO를 대체하는 게 아니라 그 위에 얹는 레이어임.** 별도로 분리하거나 경쟁적 우선순위로 취급하는 건 "2026년 가장 비싼 전략적 실수"라고 불리고 있음
2. **기존 SEO 실무를 버리면 안 됨.** 고품질 콘텐츠, 기술적 위생, 키워드 리서치, 백링크 프로필은 AI 가시성에도 핵심임
3. **패러다임 전환**: "블루링크 1위"가 목표가 아니라 "AI 답변 안에서 인용되는 것"이 목표가 된 거임
4. **AI 권위는 누적적임** — 매 분기 미룰수록 먼저 시작한 경쟁사의 구조적 우위가 커짐
5. **측정 체계를 새로 짜야 함.** 전통 SEO 지표만으로는 GEO 성과를 제대로 측정할 수 없음
...
- 최적 전략: **"풀스택 최적화"** — SEO 기술 토대 + AEO 형식 + GEO 권위를 다 챙기는 거임

**한국 GEO 실행 포인트:**
- 네이버 블로그, 카페, 지식iN 등 네이버 생태계 콘텐츠도 AI 브리핑 소스로 활용됨
- 한국어 콘텐츠의 구조화(정의형 문장, Q&A)가 네이버 AI 브리핑 인용에 유리함
- 글로벌 AI 검색에서 한국어 콘텐츠 인용을 원하면 영문 콘텐츠 병행 전략이 필요함

---

## 핵심 인사이트

1. **GEO는 "패시지 단위 최적화"임.** 페이지가 아니라 문단 수준에서 AI가 뽑아가기 좋은 형태(BLUF, 통계, 출처, 구조)로 콘텐츠를 설계해야 함. 책 전체가 아니라 밑줄 친 한 문단이 인용되게 만드는 거임. 구조화된 데이터가 있는 페이지는 AI 인용 확률이 2.8배 높음.
2. **llms.txt와 geo-seo-claude 같은 도구가 GEO를 실행 가능하게 만듦.** llms.txt로 AI한테 사이트 소개서를 보내고, 오픈소스 도구로 에이전시가 월 200~1200만원 받는 감사를 무료로 수행할 수 있어서 진입장벽이 확 낮아졌음.
3. **한국 시장은 네이버 AI 브리핑 + 글로벌 AI 검색의 "듀얼 GEO" 전략이 필요함.** 네이버 생태계와 글로벌 AI 플랫폼 모두에서 인용되려면 한국어 구조화 콘텐츠 + 영문 병행이 최적임.
