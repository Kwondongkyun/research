...
GEO(Generative Engine Optimization, AI한테 내 콘텐츠가 인용되게 만드는 전략)는 ChatGPT, Google AI Overviews, Perplexity 같은 생성형 AI의 답변에서 내 콘텐츠가 인용되도록 최적화하는 거임. 기존 SEO가 "검색 결과 1페이지에 올라가자"였다면, GEO는 "AI가 내 글을 인용하게 만들자"가 목표임. 2025년 GEO 시장 규모는 8.48억 달러이고, 2034년에는 335.7억 달러(CAGR 50.5%)까지 성장할 전망임. Gartner는 2026년까지 전통 검색엔진 검색량이 25% 줄어들 거라고 예측하고 있음.
...
GEO(Generative Engine Optimization)는 생성형 AI 시스템(ChatGPT, Google Gemini, Claude, Perplexity 등)이 답변할 때 내 콘텐츠를 인용하고 노출하도록 만드는 전략임.

핵심은 경쟁 구도가 완전히 달라졌다는 거임. Google은 검색 결과를 10개 보여주잖아? 그런데 LLM은 답변 하나에 평균 **2~7개 도메인만 인용**함. 비유하자면 SEO는 "10명 줄 세우기"인데, GEO는 "2~7명만 뽑기"인 거임. 뽑히지 못하면? 존재하지 않는 거나 마찬가지임.
...
RAG(검색해서 찾은 정보를 AI가 답변에 활용하는 구조)는 모든 AI 검색 시스템의 핵심임. 쉽게 말하면 도서관에서 관련 책을 찾아와서 요약해주는 사서 같은 구조임. 작동 방식은 이러함:

1. **검색(Retrieval)**: 사용자 질문을 받으면 검색 인덱스에서 관련 웹페이지를 찾아옴
2. **처리(Processing)**: 찾아온 문서를 작은 단위(단락, 논리 블록)로 쪼갬
3. **시맨틱 검색(글자가 아닌 의미로 검색)**: 텍스트를 벡터 임베딩(텍스트를 숫자로 변환해서 의미적 유사도를 계산하는 것)으로 바꾸고, 의미가 비슷한 조각들을 골라냄
4. **생성(Generation)**: 골라낸 조각들을 LLM한테 넘겨서 답변을 만들어냄

### 플랫폼별 특성

#### ChatGPT Search
- **인덱스**: Bing 검색 API로 웹 콘텐츠를 검색함
- **크롤러**: GPTBot(학습용), OAI-SearchBot(실시간 검색용), ChatGPT-User(사용자 요청 시)
- **인용 특성**: Wikipedia가 전체 인용의 7.8%로 1등임. 백과사전처럼 사실 기반 콘텐츠를 좋아함
- **트래픽**: AI 레퍼럴 트래픽의 **87.4%**를 차지. 압도적임

#### Google AI Overviews
- **인덱스**: Google 자체 검색 인덱스 + Knowledge Graph 사용
- **선택 기준**:
  - E-E-A-T(경험·전문성·권위·신뢰) 콘텐츠의 **96%**가 검증된 E-E-A-T 신호를 가진 소스에서 선택됨
  - 의미적 완전성 8.5/10 이상이면 인용 확률 **4.2배** 증가
  - 멀티모달 콘텐츠(이미지, 영상 포함)면 선택률 **156%** 증가
  - 15개 이상 엔티티 인식 시 선택 확률 **4.8배** 증가
- **인용 특성**: Reddit가 최다 인용(2.2%). 134~167단어 정도의 자기완결적 단위를 좋아함
- **트래픽 영향**: 인용된 페이지는 인용 안 된 경쟁자보다 오가닉 클릭이 **35%** 더 많음

#### Perplexity AI
- **인덱스**: 실시간으로 웹을 크롤링하고 API로 수집함. 정적 인덱스에 의존하지 않는 게 특징임
- **크롤러**: PerplexityBot(인덱싱), Perplexity-User(실시간 검색)
- **기술**: 어휘적 방법과 시맨틱 인덱싱을 결합한 하이브리드 검색 파이프라인을 씀
- **인용 특성**: Reddit가 최다(6.6%). 인라인 번호 인용으로 원본 검증이 가능함
- **주의**: robots.txt(출입 규칙표)를 우회하는 비공개 크롤러를 쓴다는 보고가 있음 (Cloudflare 발견)

#### Claude (Anthropic)
- **크롤러**: anthropic-ai(학습), ClaudeBot(채팅 인용), Claude-SearchBot(웹 검색)
- 웹 검색 기능이 비교적 최근에 추가돼서 인용 패턴에 대한 공개 데이터가 아직 적음
...
- **2023년 11월**: Princeton University, Georgia Tech, Allen Institute for AI, IIT Delhi 연구진이 "GEO: Generative Engine Optimization" 논문을 발표했음 (arXiv:2311.09735)
- **핵심 발견**: GEO 기법을 적용하면 생성 엔진 응답에서 가시성을 최대 **40%**까지 올릴 수 있다는 걸 증명했음
- **2024년 8월**: KDD 2024 (제30회 ACM SIGKDD 학회)에서 정식으로 발표됐음
...
이 세 전략은 서로 배타적인 게 아님. 2025년 이후 최적 전략은 **SEO + AEO + GEO 통합**임:
- **SEO**: 기초적인 발견 가능성을 깔아주고
- **AEO**(AI가 바로 답변할 수 있게 최적화): 빠른 답변 노출을 확보하고
- **GEO**: 장기적인 신뢰와 인용을 쌓아가는 거임
...
GitHub의 **ai.robots.txt** 프로젝트가 커뮤니티 기반으로 AI 크롤러 목록을 관리하고 있음. 여기서 주의할 점이 있는데, AI 크롤러를 차단하면 학습 데이터 스크래핑은 막을 수 있지만, **동시에 인용 기회도 날아감**. 그래서 대부분의 비즈니스는 크롤러를 허용하되 특정 디렉토리만 제한하는 전략이 권장됨.
...
1. **GEO는 SEO를 대체하는 게 아니라 확장하는 거임.** SEO + AEO + GEO 통합 전략이 2026년 최적해이고, 어느 하나를 버리는 건 "가장 비싼 전략적 실수"가 됨.
2. **AI 검색의 경쟁은 극도로 압축적임.** 10개 블루 링크에서 2~7개 인용으로 줄어들었으니까, 뽑히지 못하면 존재하지 않는 거나 마찬가지임. 초기 진입자 이점이 점점 따라잡기 어려워지고 있음.
3. **플랫폼 간 인용 도메인 중복률은 불과 11%임.** ChatGPT, Google AI Overviews, Perplexity가 각각 다른 소스를 좋아하니까 플랫폼별 맞춤 전략이 필수임.
